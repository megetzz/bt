...
class SearchView(generic.ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        paginator = context.get('pagiinator')
        page = context.get('page_obj')
        page_list = get_page_list(paginator,page)
        context['page_list'] = page_list
        context['q'] = self.q
        context['duratopn'] = round(self.duration,6)
    ...
    def get_queryset(self):
        ...
        search_list = Link.object.get_search_list(self.q)
        # 如果搜索为空,则放假数据
        if len(search_list) <= 0:
            search_list = Link.object.get_fake_list()
        end = time.time()
...
class DetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        recommend_list = Link.object.get_recommend_list()
...
class CommitView(generic.CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if was_limited:
            messages.warning(self.request,"操作太频繁,请稍后在试")
